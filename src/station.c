...
static void* th_worker(void* arg){
    ...
    for(;;){
        if(atomic_load(cx->done) && cx->q->size==0) break;
        ...
        if(cx->idx == 0){
            ...
            if (p.epoch_s == 0.0) p.epoch_s = *cx->epoch_value;
        ...
        if (cx->cfg.policy == POL_FCFS){
            ...
            if (work > 0){
                ...
                // ⬇️ importante: marcar salida aquí también
                p.t_out_s[cx->idx] = s1;
            }
            *rem = 0;
        } else {  // ======= POL_RR =======
        ...
        // ⬇️ Marcar salida (común a FCFS y RR) solo si ya no queda remanente
...
static void run_station_with_queue(int in_fd, int out_fd, int idx, StationConfig cfg,
    ...
    char role[16]; snprintf(role,sizeof(role),"station%d", idx+1);
    ...
    StationCtx cx = {
        .in_fd=in_fd, .out_fd=out_fd, .idx=idx, .cfg=cfg,
        .q=&q, .done=&done, .epoch_set=epoch_set, .epoch_value=epoch_value
    };
    ...
    if (idx == 2) {
        ...
        if (g_n_done_total > 0) {
            ...
            printf("Orden final de procesamiento: ");
            for (int i=0; i<g_finish_len; ++i)
                printf("P%d%s", g_finish_order[i], (i+1<g_finish_len)?" -> ":"\n");
        } else {
    ...
    if(out_fd>=0 && idx<2) close(out_fd); // E3 no tiene siguiente
    LOG(role, "fin");
    exit(0);
}
